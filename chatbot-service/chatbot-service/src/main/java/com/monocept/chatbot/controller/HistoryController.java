...
public class HistoryController {
    ...
    public ResponseEntity<MasterResponse<Page<MessageDto>>> getMessagesFromLast90Days(@Valid @RequestBody MessageRequest messageRequest) {
        //1. if redis not null 2. get paginated data from redis.
       // if redis data exausted then paginated data from DB
        try {
   ...
   // @Autowired
    ...
    //public List<MessageDto> getHistory(){
       //List<MessageDto> messages= service.getPaginatedMessages("user123@example.com",1,10);
    ...
    public ResponseEntity<MasterResponse<Page<MessageDto>>> chatHistory(@Valid @RequestBody MessageRequest messageRequest) {
        try {
            ...
            if (messages != null && !messages.isEmpty()) {
                // Success case, messages found
                response = new MasterResponse<>("success",HttpStatus.OK.value(),"Messages retrieved successfully.", messages);
                // response = new MasterResponse<>("success",HttpStatus.OK.value(),"Messages retrieved successfully.", null);
