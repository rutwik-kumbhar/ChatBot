...
public interface ChatHistoryRepository extends JpaRepository<History, Long> {
  ...
  //  @Query(value = "SELECT * FROM chat_history c WHERE c.date_time >= :dateTime AND c.email = :email", nativeQuery = true)
   // List<History> findMessagesFromLast90DayswrtEmail(LocalDateTime dateTime, String email);
    ...
    @Query("SELECT new com.monocept.chatbot.ChatDTO.HistoryDTO(c.msgId, c.msg, c.messageTo, c.dateTime, c.replyId, c.type, c.mediaUrl, c.activity) FROM History c WHERE c.dateTime >= :dateTime AND c.email = :email order by dateTime DESC" )
    List<HistoryDTO> findMessagesFromLast90DayswrtEmail(@Param("dateTime") LocalDateTime dateTime, @Param("email") String email);
